	...
	if(dwVer == DX_D3D11)
	{
		...
		if(CDXSwapChian::GetDesc(pDeviceOrSwapChain, &desc) == S_OK)
		{
			...
			dwFormat = desc.BufferDesc.Format;
         		...
         		LPVOID pDevice = NULL;
         		((IDXGISwapChain)pSwapChain)->GetDevice(IID_ID3D11Device, &pDevice);
        ID3DBlob *pBblob=NULL;
        ID3DBlob *pErrorBlob=NULL;
        D3DX11CompileFromFile( L"c:\\effect.fx", NULL, NULL, NULL, "fx_5_0", D3D10_SHADER_SKIP_VALIDATION|D3D10_SHADER_ENABLE_BACKWARDS_COMPATIBILITY, NULL, NULL, &pBblob, &pErrorBlob, NULL );
        D3DX11CreateEffectFromMemory( pBblob->GetBufferPointer(), pBblob->GetBufferSize(), NULL, pDevice, &g_pEffect ) ;
	}
