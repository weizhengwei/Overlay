...
void CCoreHook::RenderOverlay(DWORD dwVer, LPVOID pDeviceOrSwapChain)
{
 			...
 			if(pDeviceContext)
 			{
 				...
 				if(!g_pVertexShader)
 				{
 					...
 					do 
 					{
 						...
 						D3D11_INPUT_ELEMENT_DESC layout[] =
 						{
 							...
 							{ "TEXCOORD", 0, 16/*DXGI_FORMAT_R32G32_FLOAT*/, 0, DWORD(-1)/*D3D11_APPEND_ALIGNED_ELEMENT*/, 0/*D3D11_INPUT_PER_VERTEX_DATA*/, 0 },
 						};
 						UINT numElements = 2;
 						// Create the input layout
 						hr = CDXDevice11::CreateInputLayout( pDevice, layout, numElements, (const void *)(INT_PTR)CDXBlob::GetBufferPointer(pVSBlob), 
 						...
 						bd.Usage = 0/*D3D11_USAGE_DEFAULT*/;
 						...
 						rasterizerDesc.CullMode = 3/*D3D11_CULL_BACK*/;
 						...
 						hr = CDXDevice11::CreateBlendState(pDevice, &blendDesc, &g_pBlendState);
 						if(hr != S_OK)
 						{
 							break;
 						}
 				...
 				if(bResize || !m_pOSTexture)
 				{
 					...
 					if(m_pOSTexture)
 					{
 						...
 						if(S_OK != CDXDevice11::CreateShaderResourceView(pDevice, m_pOSTexture, &srvd, &g_pTextureRV))
                         {
                             MessageBox(0,0,0,0);
                         }
 					}
 ...
 //                                      *(PBDest+3)=*(PBSrc+3);
                        ...
                        UINT format = 0;
                        UINT off = 0;
                        CDXDeviceContext11::IAGetIndexBuffer(pDeviceContext, &pIndexBuffer, &format, &off);

                        LPVOID pOldGShader = NULL;
                        LPVOID pOldHShader = NULL;
                        LPVOID pOldDShader = NULL;
                        LPVOID pOldCShader = NULL;
                        CDXDeviceContext11::GSGetShader(pDeviceContext, &pOldGShader, NULL, NULL);
                        CDXDeviceContext11::HSGetShader(pDeviceContext, &pOldHShader, NULL, NULL);
                        CDXDeviceContext11::DSGetShader(pDeviceContext, &pOldDShader, NULL, NULL);
                        CDXDeviceContext11::CSGetShader(pDeviceContext, &pOldCShader, NULL, NULL);


                        LPVOID pRenderTargetView = NULL;
                        LPVOID pDepthStencilView = NULL;

                        CDXDeviceContext11::OMGetRenderTargets(pDeviceContext, 1, &pRenderTargetView, &pDepthStencilView);

                        LPVOID pRenderTargetView1 = NULL;
                        LPVOID pDepthStencilView1 = NULL;
                        LPVOID pView1 = NULL;
                        CDXDeviceContext11::OMGetRenderTargetsAndUnorderedAccessViews(pDeviceContext, 1, &pRenderTargetView1, &pDepthStencilView1, 0, 1, &pView1);
 ...
 //                         LPVOID pDepthStencilView = NULL;
 // 
 //                         CDXDeviceContext11::OMGetRenderTargets(pDeviceContext, 0, &pRenderTargetView, &pDepthStencilView);
 // 
 //                         //D3D11_RENDER_TARGET_VIEW_DESC pDesc;
 //                         //ID3D11RenderTargetView::GetDesc(pRenderTargetView, &pDesc);
 //                         // Clear the back buffer
 //                         float ClearColor[4] = { 0.0f, 0.125f, 0.3f, 1.0f }; // red,green,blue,alpha
 //                         //第一个程序已经说了，这个函数时将目标对象的所有元素都赋成一个属性
 //                         CDXDeviceContext11::ClearRenderTargetView(pDeviceContext, pRenderTargetView, ClearColor );
                        ...
                        CDXDeviceContext11::ClearState(pDeviceContext);
                         ...
                         LPVOID ppIndexBuffers[14] = {0};
                         CDXDeviceContext11::VSSetConstantBuffers(pDeviceContext, 0, 14, ppIndexBuffers);
                         ...
                         LPVOID pIndexBuffers[14] = {0};
                         CDXDeviceContext11::PSSetConstantBuffers(pDeviceContext, 0, 14, pIndexBuffers);
