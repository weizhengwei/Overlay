...
public:
	...
	HRESULT ResolveLink(CString LnkName, UINT SpecialFolder,CString SubFolder,HWND hwnd, CString &LnkPath, CString &LnkDescription);
	/* looks if the link, with the given name in the special folder, already exists: */
	BOOL isLinkAvailable(CString LnkName, UINT SpecialFolder, CString SubFolder=_T(""));
	...
	BOOL CreateShortCut(CString LnkTarget, CString LnkName, UINT SpecialFolder = CSIDL_DESKTOP,CString SubFolder=_T(""), CString LnkDescription =_T(""), CString IconLocation =_T(""), UINT IconIndex = 1);
	/*! Use this routine to delete any Shortcut */
	BOOL DeleteShortCut(CString LnkName, UINT SpecialFolder,CString SubFolder=_T(""));
private:
