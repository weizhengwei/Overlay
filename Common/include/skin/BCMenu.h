...
public:
	BCMenu(int nFontWeight = FW_REGULAR); 
	virtual ~BCMenu();

	HHOOK m_hHook ; ///remove pop up menu frame
	static WNDPROC m_oldWndProc ; ///remove pop up menu frame

	static LRESULT CALLBACK WindowsHook(int code, WPARAM wParam, LPARAM lParam) ; ///remove pop up menu frame
	static LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) ; ///remove pop up menu frame
