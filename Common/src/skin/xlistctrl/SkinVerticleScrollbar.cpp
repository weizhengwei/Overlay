...
#include "stdafx.h"
#include "skin/xlistctrl/SkinVerticleScrollbar.h"
...
void CSkinVerticleScrollbar::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	int nHeight = clientRect.Height() - 20;
	

	CRect rectUpArrow(0,0,18,20);
	CRect rectDownArrow(0,nHeight,18,nHeight+20);
	CRect rectThumb(0,nThumbTop,18,nThumbTop+26);
...
//	UpdateThumbPosition();
	...
	CRect thumbNew = rectThumb ;
	thumbNew.top = nThumbTop ;
	if(thumbNew.PtInRect(point))
	{
		bInChannel = false;
...
void CSkinVerticleScrollbar::Draw()
{
	...
	memDC.StretchBlt(clientRect.left,clientRect.top,18,20,&bitmapDC,0,0,18,16,SRCCOPY);
...
/*	bitmap.LoadBitmap(IDB_VERTICAL_SCROLLBAR_UPARROW);
	...
	bitmap.LoadBitmap(IDB_VERTICAL_SCROLLBAR_CHANNEL);
	pOldBitmap = bitmapDC.SelectObject(&bitmap);
	nHeight = clientRect.Height() - 40;

	memDC.FillSolidRect(clientRect.left, clientRect.top+20, 18,nHeight,RGB(46,46,46));//StretchBlt(clientRect.left, clientRect.top+20, 18,nHeight,&bitmapDC, 0,0, 18, 24, SRCCOPY);
...
/*	bitmap.LoadBitmap(IDB_VERTICAL_SCROLLBAR_UPARROW);
	...
	pOldBitmap = bitmapDC.SelectObject(&bitmap);
	memDC.StretchBlt(clientRect.left,clientRect.bottom-20,18,20,&bitmapDC,0,0,18,16,SRCCOPY);
	bitmapDC.SelectObject(pOldBitmap);
	bitmap.DeleteObject();
	pOldBitmap = NULL;
	...
	if(pList->GetScrollLimit(SB_VERT) != 0)
	{
		...
		pOldBitmap = bitmapDC.SelectObject(&bitmap);
		memDC.BitBlt(clientRect.left,clientRect.top+nThumbTop,18,26,&bitmapDC,0,0,SRCCOPY);
		bitmapDC.SelectObject(pOldBitmap);
		bitmap.DeleteObject();
		pOldBitmap = NULL;
	}
...
void CSkinVerticleScrollbar::LimitThumbPosition()
{
	...
	if(nThumbTop+32 >= (clientRect.Height()-22))
	{
		nThumbTop = clientRect.Height()-44;
	}

	if(nThumbTop < (clientRect.top+22))
	{
		nThumbTop = clientRect.top+22;
	}
}
