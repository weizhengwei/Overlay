...
#include "stdafx.h"
#include "skin/xlistctrl/XListCtrl.h"
#include "skin/xlistctrl/SortCStringArray.h"
...
LRESULT CXListCtrl::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
void CXListCtrl::PositionScrollBars()
{
	...
	int nDialogFrameHeight = 14;
	int nDialogFrameWidth = 17;
	...
	CRect vBar(windowRect.right-nDialogFrameWidth,windowRect.top,windowRect.right,windowRect.bottom);
...
void CXListCtrl::OnShowWindow(BOOL bShow,UINT nStatus)
{
/*	if(bShow)
...
void CXListCtrl::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
...
void CXListCtrl::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
//	m_SkinVerticleScrollbar.UpdateThumbPosition();
...
bool CXListCtrl:: SetBkColor( COLORREF cr)
{
	m_crWindow=cr;
	return true;
}
...
void CXListCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	ClientToScreen(&pt) ;
//	m_SkinVerticleScrollbar.ScreenToClient(&pt) ;
//	m_SkinVerticleScrollbar.SendMessage(WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(pt.x, pt.y)) ;
...
void CXListCtrl::GetColors()
{
	...
	m_crHighLight           = RGB(119,73,20);//::GetSysColor(COLOR_HIGHLIGHT);
	m_crHighLightText       = RGB(149,149,149) ;//::GetSysColor(COLOR_HIGHLIGHTTEXT);
...
void CXListCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	ClientToScreen(&pt) ;
//	m_SkinVerticleScrollbar.ScreenToClient(&pt) ;
//	m_SkinVerticleScrollbar.SendMessage(WM_MOUSEMOVE, 0, MAKELPARAM(pt.x, pt.y)) ;
...
void CXListCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	ClientToScreen(&pt) ;
//	m_SkinVerticleScrollbar.ScreenToClient(&pt) ;
//	m_SkinVerticleScrollbar.SendMessage(WM_LBUTTONUP, MK_LBUTTON, MAKELPARAM(pt.x, pt.y)) ;
