...
#include "stdafx.h"
#include "skin/xlistctrl/XHeaderCtrl.h"
#include "skin/xlistctrl/memdc.h"
...
{
	m_cr3DHighLight   =  RGB(55,55,55);//::GetSysColor(COLOR_3DHIGHLIGHT);
	m_cr3DShadow      =  RGB(0x72,0x72,0x72);//::GetSysColor(COLOR_3DSHADOW);
	m_cr3DFace        =  RGB(0x21,0x21,0x21);//::GetSysColor(COLOR_3DFACE);
	m_crBtnText       = ::GetSysColor(COLOR_BTNTEXT);//RGB(0x225,0x225,0x225);
	m_pListCtrl       = NULL;			//+++
	m_nFormat         = DT_DEFAULT;		//+++
	m_rgbText         = RGB(97,97,97);//m_crBtnText;	//+++
...
/*					pDC->SelectObject(&penHighLight);
	...
	if (iWidth > 0)
	{
		...
		pDC->Draw3dRect(rectClient, m_cr3DHighLight, m_cr3DHighLight);
	}
...
///////////////////////////////////////////////////////////////////////////////
// DrawItem
void CXHeaderCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CBrush   backbrush(RGB(0x21,0x21,0x21));
...
//	rect.right -= ((iWidth = DrawBitmap(pDC, rect, lphdi, pBitmap, &BitmapInfo, TRUE)) != 0) ?	iWidth + m_iSpacing : 0;
	...
	pDC->FillSolidRect(rect,RGB(0x21,0x21,0x21));
...
void CXHeaderCtrl::OnSysColorChange() 
{
	...
	m_cr3DHighLight = RGB(55,55,55);//::GetSysColor(COLOR_3DHIGHLIGHT);
	m_cr3DShadow    = RGB(0x25,0x25,0x25);//::GetSysColor(COLOR_3DSHADOW);
	m_cr3DFace      = RGB(0x21,0x21,0x21);//::GetSysColor(COLOR_3DFACE);
