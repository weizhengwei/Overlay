...
inline int CUIPictureEx::TGIFControlExt::GetPackedValue(enum ControlExtValues Value)
{
...
inline int CUIPictureEx::TGIFLSDescriptor::GetPackedValue(enum LSDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
inline int CUIPictureEx::TGIFImageDescriptor::GetPackedValue(enum IDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
BEGIN_MESSAGE_MAP(CUIPictureEx, CStatic)
	//{{AFX_MSG_MAP(CUIPictureEx)
	ON_WM_DESTROY()
	ON_WM_PAINT()
	ON_WM_CTLCOLOR()
	//}}AFX_MSG_MAP
...
BOOL CUIPictureEx::Load(HGLOBAL hGlobal, DWORD dwSize)
{
	...
	// do the default processing
		...
		m_PictureSize.cx = MulDiv(hmWidth, GetDeviceCaps(hDC,LOGPIXELSX), 2540) - 1;
	...
	else
	{
		...
		m_PictureSize.cx = m_pGIFLSDescriptor->m_wWidth-1;
...
BOOL CUIPictureEx::Draw()
{
	if (!m_bIsInitialized)
	{
		TRACE(_T("Call one of the CUIPictureEx::Load() member functions before calling Draw()\n"));
		return FALSE;
	};
...
SIZE CUIPictureEx::GetSize() const
{
	return m_PictureSize;
}

BOOL CUIPictureEx::Load(LPCTSTR szFileName)
{
	BOOL bRet = FALSE ;

	__try {

		bRet = _Load(szFileName) ;
...
BOOL CUIPictureEx::_Load(LPCTSTR szFileName)
{
	ASSERT(szFileName);
...
BOOL CUIPictureEx::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
...
void CUIPictureEx::ResetDataPointer()
{
...
BOOL CUIPictureEx::SkipNextGraphicBlock()
{
...
UINT CUIPictureEx::GetSubBlocksLen(UINT nStartingOffset) const
{
	UINT nRet = 0;
...
enum CUIPictureEx::GIFBlockTypes CUIPictureEx::GetNextBlock() const
{
...
BOOL CUIPictureEx::SkipNextBlock()
{
	if (!m_pRawData) return FALSE;
...
int CUIPictureEx::GetNextBlockLen() const
{
...
UINT WINAPI CUIPictureEx::_ThreadAnimation(LPVOID pParam)
{
	ASSERT(pParam);
	CUIPictureEx *pPic = reinterpret_cast<CUIPictureEx *> (pParam);
...
void CUIPictureEx::ThreadAnimation()
{
	...
	while (!m_bExitThread)
	{
		...
		//   3 - restore to previous
			...
			long hmWidth;
			long hmHeight;
			if ((m_arrFrames.size() <= m_nCurrFrame) ||  !m_arrFrames[m_nCurrFrame].m_pPicture)
...
void CUIPictureEx::Stop()
{
	m_bIsPlaying = FALSE;
	m_bExitThread = TRUE;
	//Sleep(200) ; //sleep for a while to avoid confliction operation on m_arrFrames.
...
HGLOBAL CUIPictureEx::GetNextGraphicBlock(UINT *pBlockLen, 
...
BOOL CUIPictureEx::IsGIF() const
{
	return m_bIsGIF;
}

BOOL CUIPictureEx::IsAnimatedGIF() const
{
	return (m_bIsGIF && (m_arrFrames.size() > 1));
}

BOOL CUIPictureEx::IsPlaying() const
{
	return m_bIsPlaying;
}

int CUIPictureEx::GetFrameCount() const
{
	if (!IsAnimatedGIF())
		return 0;
...
COLORREF CUIPictureEx::GetBkColor() const
{
	return m_clrBackground;
}

void CUIPictureEx::OnPaint() 
{
...
BOOL CUIPictureEx::PrepareDC(int nWidth, int nHeight)
{
	...
	FillRect(m_hMemDC,&rect,/*(HBRUSH)(COLOR_WINDOW)*/(HBRUSH)(HOLLOW_BRUSH));
...
void CUIPictureEx::OnDestroy() 
{
	Stop();	
	CStatic::OnDestroy();
}

void CUIPictureEx::SetBkColor(COLORREF clr)
{
...
#ifdef GIF_TRACING
void CUIPictureEx::WriteDataOnDisk(CString szFileName, HGLOBAL hData, DWORD dwSize)
{
	CFile file;
...
void CUIPictureEx::EnumGIFBlocks()
{
	enum GIFBlockTypes nBlock;
...
BOOL CUIPictureEx::SetPaintRect(const RECT *lpRect)
{
	return CopyRect(&m_PaintRect, lpRect);
}

BOOL CUIPictureEx::GetPaintRect(RECT *lpRect)
{
	return CopyRect(lpRect, &m_PaintRect);
}

HBRUSH CUIPictureEx::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
