...
void CSkinScrollBar::UpdateThumbPosition()
{
	CRect clientRect;
	GetClientRect(&clientRect);

	double nPos =m_pOwnerWnd->GetScrollPos(SB_VERT);
	double nMax = m_pOwnerWnd->GetScrollLimit(SB_VERT);
	double nHeight = (clientRect.Height()-98);
	double nVar = nMax;
}

void CSkinScrollBar::LimitThumbPosition()
{
	CRect clientRect;
	GetClientRect(&clientRect);
}
