...
void CGdipButton::ReleaseReSource()
{
	__try
	{
		if(m_pStdImage) 
...
BOOL CGdipButton::LoadImage(LPCTSTR lpszFilePathName) 
{
	m_pStdImage = new CGdiPlusBitmapResource;
	return (m_pStdImage->Load(lpszFilePathName));
}
...
{
	if(!m_bHaveBitmaps) //remove by shenhui to allow button for re-drawn
	{
		if(!m_pStdImage)
		{
		...
		CMemDCGdiplus pDC(pScreenDC, &rect);
		...
		if (m_dcStd.m_hDC == NULL)
		{
			...
			// standard image hot
			//if(m_dcStdD.m_hDC == NULL)
			//{
			//	PaintBk(pDC);

			//	ColorMatrix HotMat = {	0.5f, 0.00f, 0.00f, 0.00f, 0.00f,
			//							0.00f, 0.5f, 0.00f, 0.00f, 0.00f,
			//							0.00f, 0.00f, 0.5f, 0.00f, 0.00f,
			//							0.00f, 0.00f, 0.00f, 0.5f, 0.00f,
			//							0.05f, 0.05f, 0.05f, 0.00f, 1.00f	};
			...
			//	m_dcStdD.CreateCompatibleDC(pDC);
			//	bmp.CreateCompatibleBitmap(pDC, rect.Width(), rect.Height());
			//	pOldBitmap = m_dcStdD.SelectObject(&bmp);
			//	m_dcStdD.BitBlt(0, 0, rect.Width(), rect.Height(), pDC, 0, 0, SRCCOPY);
			//	bmp.DeleteObject();
			//}
...
void CGdipButton::SetBkGnd(CDC* pDC)
{
	...
	/*
	** added by shenhui to re-draw the background of button. 20101207
	*/ 
	m_dcBk.DeleteDC() ;
	m_dcStd.DeleteDC() ;
	m_dcStdH.DeleteDC() ;
