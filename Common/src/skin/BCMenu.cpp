...
#include "skin/BCMenu.h"        // BCMenu class declaration
...
BOOL BCMenu::bRemoveMenuBolder = FALSE ; ///avoid popup menu style incorrect when popping up system menu.
...
UINT BCMenu::xp_drawmode=BCMENU_DRAWMODE_ORIGINAL;
...
WNDPROC BCMenu::m_oldWndProc = NULL ; ///remove pop up menu frame	

CImageList BCMenu::m_AllImages;
CArray<int,int&> BCMenu::m_AllImagesID;
int BCMenu::m_iconX = 0;
int BCMenu::m_iconY = 0;
...
LRESULT BCMenu::WindowsHook (int code, WPARAM wParam, LPARAM lParam)
{
	...
	{ 
		...
		//whether a menu window
		if ( Count != 6 || _tcscmp (sClassName, _T("#32768")) != 0 ) 
		{ 
			break; 
		} 

		if (!bRemoveMenuBolder)
		{//if popup window taskbar menu, do not change style
			break ;
		}

		WNDPROC lastWndProc = (WNDPROC)GetWindowLong(hWnd, GWL_WNDPROC); 
		if (lastWndProc != (WNDPROC)&BCMenu::WndProc)
		{ 
			//replace menu window proc
			SetWindowLong(hWnd, GWL_WNDPROC, (long)BCMenu::WndProc); 

			//save old menu window proc
...
LRESULT CALLBACK BCMenu::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	LRESULT lResult ;

	switch (message) 
	{ 
	case WM_CREATE: //remove WS_BORDER, WS_EX_DLGMODALFRAME and WS_EX_WINDOWEDGE style of menu
		{ 
	...
	case WM_PRINT: //prevent painting of non-client area 
		return CallWindowProc (m_oldWndProc, hWnd, WM_PRINTCLIENT, wParam, lParam); 
		break; 
	case WM_WINDOWPOSCHANGING: //subtract frame size because system will add for you.
		{ 
			//make menu size smaller
...
BCMenu::BCMenu(int nFontWeight)
{
	...
	m_bitmapBackground=RGB(33,33,33); //gray
	m_clrBg=RGB(33,33,33);
	m_clrSel=RGB(54,54,54);
	m_clrText=RGB(223,223,223);
	m_clrEdge=RGB(33,33,33);
	m_iWidth=0;

	m_fontText.Detach( );
	m_fontText.CreateFont(-12, 0, 0, 0, nFontWeight, FALSE, FALSE,0,0,0,0,0,0, _T("Tahoma"));
	...
	//if (AfxGetApp())
	//{
	//	m_hHook = ::SetWindowsHookEx(WH_CALLWNDPROC, (HOOKPROC)BCMenu::WindowsHook, AfxGetApp()->m_hInstance, ::GetCurrentThreadId());
	//}
...
/*	if(pDC->GetDeviceCaps(RASTERCAPS) & RC_PALETTE)DrawItem_WinXP(lpDIS);
	else{
		if(IsWinXPLuna()){
...
#endif
	CRect rect,rect2;
	if (((BCMenuData*)(lpDIS->itemData)) == NULL) return ;
	UINT state = (((BCMenuData*)(lpDIS->itemData))->nFlags);

	COLORREF m_newclrBack=m_clrBg;
	COLORREF m_clrBack=m_clrBg;
...
//	m_clrBack=DarkenColor(m_clrBack,0.02);
	...
	if(lpDIS->itemState & ODS_GRAYED&&!xp_select_disabled){
		...
		pFont = pDC->SelectObject (&m_fontText);
#endif
	...
	if(state & MF_SEPARATOR){
		...
		Graphics gDC(lpDIS->hDC) ;

		Color cFrame(72,71,72) ;
		Pen pFrame(cFrame) ;
		gDC.DrawRectangle(&pFrame, 0, 0, lpDIS->rcItem.right-1, lpDIS->rcItem.bottom*m_MenuList.GetCount()-1) ;

		BOOL standardflag=FALSE,selectedflag=FALSE,disableflag=FALSE;
		BOOL checkflag=FALSE;
		COLORREF crText = m_clrText;
		COLORREF crSelect = m_clrSel;
		...
		if(state&ODS_SELECTED){ // draw the down edges
			...
			pDC->FillRect (CRect(rect.left+1,rect.top+1, rect.right-1, rect.bottom-1),&m_brSelect);
...
//			rect2.SetRect(rect.left,rect.top,rect.left+barwidth,rect.bottom);
			rect2.SetRect(rect.left+1,rect.top+1,rect.left+barwidth-1,rect.bottom-1);
			CPen *pOldPen = pDC->SelectObject (&m_penBack);
			if (m_MenuList[0]->nID == lpDIS->itemID)
			{
				pDC->FillRect (CRect(rect.left+1,rect.top+1, rect.right-1, rect.bottom),&m_brBackground);
			}
			else if (m_MenuList[m_MenuList.GetCount()-1]->nID == lpDIS->itemID)
			{
				pDC->FillRect (CRect(rect.left+1,rect.top, rect.right-1, rect.bottom-1),&m_brBackground);
			}
			else
			{
				pDC->FillRect (CRect(rect.left+1,rect.top, rect.right-1, rect.bottom),&m_brBackground);
			}
			//pDC->FillRect (rect2,&m_newbrBackground);
			pDC->SelectObject (pOldPen);
			...
			//pDC->Draw3dRect (CRect(rect.left+1,rect.top, rect.right-1, rect.bottom-1),m_clrBack,m_clrBack);
			//pDC->Draw3dRect (rect2,m_newclrBack,m_newclrBack);
		}
		...
		if(!strText.IsEmpty()){
			...
			if(!(lpDIS->itemState & ODS_GRAYED)){
				pDC->SetTextColor(/*crText*/0xacacac);
			...
			pDC->SetBkMode( TRANSPARENT);//iOldMode );
		}
...
#ifdef BCMENU_USE_MEMDC
	if(pFont)pDC->SelectObject (&m_fontText); //set it to the old font
...
{
	if (lpMIS == NULL || ((BCMenuData*)(lpMIS->itemData)) == NULL)
	{//add this verification to avoid coreclient crash.
		return ;
	}
...
///add by shenhui
		...
		lf.lfCharSet = 0 ;//GB2312_CHARSET;//ANSI_CHARSET ;
		...
		lf.lfWeight = FW_BOLD ;	
		memcpy(lf.lfFaceName, _T("Tahoma"), 7) ;
///end
		m_fontMenu.CreateFontIndirect (&lf);
		
		// Obtain the width of the text:
		CWnd *pWnd = AfxGetApp()->GetMainWnd();            // Get main window
		if (pWnd == NULL) pWnd = CWnd::GetDesktopWindow();
		CDC *pDC = pWnd->GetDC();              // Get device context
		CFont* pFont=NULL;    // Select menu font in...
		
		if (IsNewShell())
			pFont = pDC->SelectObject (&m_fontText);// Select menu font in...
		...
		if (Win32s!=g_Shell)
			...
			size.cx += 1*(size.cx/wcslen(lpstrText));
		}
#endif    
		
		CSize t = CSize(size);
		if(IsNewShell())
			pDC->SelectObject (&m_fontText);  // Select old font in
		pWnd->ReleaseDC(pDC);  // Release the DC
		
		// Set width and height:
		
		if(IsLunaMenuStyle())lpMIS->itemWidth = m_iconX+BCMENU_PAD+8+t.cx;
		else lpMIS->itemWidth = m_iconX + t.cx + m_iconX + 28 + BCMENU_GAP;
		int temp = GetSystemMetrics(SM_CYMENU)+5;
