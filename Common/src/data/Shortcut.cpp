...
and some more.....
*****************************************************************/
BOOL CShortcut::CreateShortCut(CString LnkTarget, CString LnkName, UINT SpecialFolder,CString SubFolder, CString LnkDescription, CString IconLocation, UINT IconIndex)
{
	HRESULT hr;
	CFile cfFull;
	...
	sSpecialFolder += SubFolder+ LnkName + _T(".") + _T("lnk");
	...
	if (SUCCEEDED( CoCreateInstance(CLSID_ShellLink,NULL,CLSCTX_INPROC_SERVER,IID_IShellLink,(LPVOID*) &psl)))
	{
		...
		if (SUCCEEDED(psl->QueryInterface(IID_IPersistFile, (LPVOID *)&ppf)))
		{
			...
			//MultiByteToWideChar(CP_ACP,MB_PRECOMPOSED,sSpecialFolder.GetString(),-1,wsz,MAX_PATH);

			/* Call IShellLink::SetIconLocation with the file containing the icon and the index of the icon */
			if(!IconLocation.IsEmpty())
			{
				hr = psl->SetIconLocation(IconLocation, IconIndex);	
			}

			if(SUCCEEDED(ppf->Save(sSpecialFolder.GetString(), TRUE)))
			{
				bRet = TRUE;
			}
...
and some more.....
*****************************************************************/
BOOL CShortcut::DeleteShortCut(CString LnkName, UINT SpecialFolder,CString SubFolder)
{
	CFile cfFull;
	CString sExePath, sExe, sSpecialFolder;
	TCHAR *chTmp = sExePath.GetBuffer(MAX_PATH);
	...
	sExe = cfFull.GetFileName();
	sExe.Delete(sExe.Find(_T(".")) + 1, 3);
	sSpecialFolder += SubFolder + LnkName + _T(".lnk");
	...
	if(sSpecialFolder.Find(_T('\0'))!=sSpecialFolder.GetLength())
	{
...
*********************************************/
BOOL CShortcut::isLinkAvailable(CString LnkName, UINT SpecialFolder,CString SubFolder)
{
	...
	// Work with the special folder's path (contained in szPath)
	sSpecialFolder += _T("\\") + SubFolder + LnkName + _T(".lnk");
...
\returns a HRESULT \n
*********************************************/
HRESULT CShortcut::ResolveLink(CString LnkName, UINT SpecialFolder, CString SubFolder, HWND hwnd, CString &LnkPath, CString &LnkDescription)
{
	HRESULT hres;     
	IShellLink* psl;
	TCHAR *szGotPath = LnkPath.GetBuffer(MAX_PATH); 
	TCHAR *szDescription = LnkDescription.GetBuffer(MAX_PATH);
	CString sLnkFile, sSpecialFolder,sLong;
	WIN32_FIND_DATA wfd;  
	...
	// build a linkfile:
	sLnkFile = sSpecialFolder + SubFolder+ LnkName + _T(".lnk");
	...
	if(SUCCEEDED(hres)) 
	{
		...
		{ 
			...
			//MultiByteToWideChar(CP_ACP,0,sLnkFile.GetString(),-1,wsz, MAX_PATH);
			// Load the shortcut. 
			hres = ppf->Load(sLnkFile.GetString(), STGM_READ); 
...
int CShortcut::ShortToLongPathName(CString sShortPath, CString &sLongPath)
{
	...
	if (0xffffffff == GetFileAttributes(sShortPath))
	{
		return 0;
	}
	...
	// Copy the short path into the work buffer and convert forward slashes to backslashes.
